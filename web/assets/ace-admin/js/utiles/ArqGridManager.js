    ...
    return {
        ...
        setSelectedRows: function(rows) {
            ...
            for (var i = 0; i < rows.length; i++) {
                var row = '' + rows[i];
                $('tbody > tr > td:nth-child(1) input[type="checkbox"]', table)
                    .filter(function(){return this.value == row}).prop("checked", true);
            }
...
var ArqGridManager = function () {

    var gridList = [];

    return {
        add: function ( gridAjax ) {
            gridList.push( {
                'id': gridAjax.getIdGrid(),
                'cgrid': gridAjax
            } );
        },
